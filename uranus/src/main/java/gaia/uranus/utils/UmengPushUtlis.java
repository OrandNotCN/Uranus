 ...
 */
public class UmengPushUtlis {

    private static UmengPushUtlis instance;

    public static synchronized UmengPushUtlis getInstance(){
        if(instance==null){
            instance = new UmengPushUtlis();
        }
        return instance;
    }
...
//		mPushAgent.setLocalNotificationIntervalLimit(false);  //默认本地通知间隔最少是10分钟
        //sdk开启通知声音
        PushAgent.getInstance(context).setNotificationPlaySound(MsgConstant.NOTIFICATION_PLAY_SDK_ENABLE);
        // sdk关闭通知声音
